...
Рекомендуем глобальную установку — команда `modxapp` будет доступна из любой директории:
...
В этом случае команда вызывается как `vendor/bin/modxapp`. Далее во всех примерах используется `modxapp` (глобальная установка).
...
=== "Headless (локально, без MODX)"
    ...
    ```bash
    modxapp setup          # скачать ядро MODX
...
=== "Локально с MODX"
    ...
    ```bash
    modxapp create mypackage
    modxapp elements mypackage    # добавить элементы в MODX
...
Создайте локальный конфиг `modxapp.json` для текущего проекта:
...
Путь к шаблонам можно задать в глобальном конфиге (`modxapp config`) или в локальном (`modxapp init`) — тогда указывать `--template` при каждом `create` не нужно. Или указать разово:
