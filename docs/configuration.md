...
|---------|------|-----------|------------|
| Глобальный | `user.config.json` (внутри пакета) | `mxbuilder config` | Данные автора, настройки по умолчанию для всех проектов |
| Локальный | `mxbuilder.json` (корень проекта) | `mxbuilder init` или автоматически при `create` | Настройки текущего проекта |
| Пакет | `package_builder/packages/<name>/config.php` | `mxbuilder create` | Конфигурация сборки конкретного пакета |

## Глобальный конфиг

Настраивается один раз командой [`mxbuilder config`](commands/config.md). Содержит данные автора, путь к шаблонам, выбор инструментов. Используется как основа при создании локального конфига.

## Локальный конфиг (mxbuilder.json)

Создаётся командой [`mxbuilder init`](commands/init.md) или автоматически при первом `mxbuilder create`. Значения берутся из глобального конфига. Можно закоммитить в репозиторий, чтобы все участники команды использовали одинаковые настройки.

## Конфигурация пакета (config.php)

Каждый пакет имеет свой файл конфигурации сборки — `package_builder/packages/<name>/config.php`. Создаётся автоматически при `mxbuilder create`.
